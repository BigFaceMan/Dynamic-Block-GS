...
from lib.models.gaussian_model_bkgd import GaussianModel
...
def seamless_merge(model_path, partition_point_cloud_dir):
    ...
    for partition in partition_scene:
        ...
        semantic_list.append(semantics[mask])
        
        fig, ax = plt.subplots()
        x_pos = xyz[mask][:, cfg.block.x_ax]
        z_pos = xyz[mask][:, cfg.block.z_ax]
    ...
    semantic_list = np.concatenate(semantic_list, axis=0)
    ...
    semantic_list = semantic_list[mask]
