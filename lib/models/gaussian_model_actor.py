...
class GaussianModelActor(GaussianModel):
    ...
    def get_features_fourier(self, frame=0):
        '''
        description : 通过IDFT得到与时间相关的feature, fourier_dim = 1 则不使用IDFT
        param [*] frame : 当前的帧, 用于解码时间
        return [*] : 得到的特征
        '''
    ...
    def construct_list_of_attributes_nfourier(self):
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
        # All channels except the 3 DC
        for i in range((self._features_dc.shape[1] // self.fourier_dim) * self._features_dc.shape[2]):
    ...
    def make_ply_nfourier(self):
        xyz = self._xyz.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        # for obj (N, d, 3) -> (N, 3, d)
        print("make ply don't use fourier!!!")
        f_total = self.get_features_fourier(0)
        f_dc = f_total[:, :1].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = f_total[:, 1:].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = self._opacity.detach().cpu().numpy()
        scale = self._scaling.detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy()
        semantic = self._semantic.detach().cpu().numpy()
        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes_nfourier()]
        print("model : {} \n dtype_full : \n {}".format(self.model_name, dtype_full))
